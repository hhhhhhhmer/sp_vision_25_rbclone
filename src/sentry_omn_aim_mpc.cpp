...
int main(int argc, char * argv[])
{
  ...
  auto mpc_thread = std::thread([&]() {
    ...
    while (!quit) {
        ...
        auto plan =  plan_short_or_long->plan(target, gs.bullet_speed, gs.yaw,  auto_aim::Planner::ShootStrategy::rbSuppressiveFire, gs.pitch, gs.yaw_vel, gs.pitch_vel);
        // 1. 设置默认值
        uint8_t name = 0;
        float tx = 0.0f;
        float ty = 0.0f;
...

    {
      ...
      if (!targets.empty()) {
        tools::draw_reprojection(
          img, short_camera_solver, targets.front(), short_camera_planner.debug_xyza);
        // 将目标放入队列供MPC线程处理
